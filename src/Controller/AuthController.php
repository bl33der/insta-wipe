...
class AuthController extends AppController
{
    ...
    public function login()
    {
        ...
        $failedAttempts = $session->read('Login.failedAttempts') ?? 0;
        $lastFailed = $session->read('Login.lastFailedTime');

        if ($failedAttempts >= 3 && $lastFailed) {
            $blockEnd = (new \DateTime($lastFailed))->add(new \DateInterval('PT30S'));
            $now = new \DateTime();
            if ($now < $blockEnd) {
                $remaining = $blockEnd->getTimestamp() - $now->getTimestamp();
                $this->Flash->error("Account is temporarily locked. Please try again in {$remaining} seconds.");
                return;
            }
        }
        ...
        if ($result && $result->isValid()) {
            ...
            $session->delete('Login.failedAttempts');
            $session->delete('Login.lastFailedTime');
        ...
        if ($this->request->is('post') && !$result->isValid()) {
            $failedAttempts++;
            $session->write('Login.failedAttempts', $failedAttempts);
            $session->write('Login.lastFailedTime', (new \DateTime())->format('Y-m-d H:i:s'));

            if ($failedAttempts >= 3) {
                $this->Flash->error('Too many failed attempts. Account is locked for 30 seconds.');
            } else {
                $remaining = 3 - $failedAttempts;
                $this->Flash->error("Email address and/or Password is incorrect. {$remaining} attempt(s) left.");
            }
        }
    }
